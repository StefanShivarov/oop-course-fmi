...
struct Answer {
    char text[Constants::MAX_TEXT_LENGTH] = "";
};

struct Question {
    char text[Constants::MAX_TEXT_LENGTH] = "";
    Answer answers[Constants::MAX_ANSWERS_AMOUNT];
...
struct Test {
    ...
    Question questions[Constants::MAX_QUESTIONS_AMOUNT];
};

void printQuestion(const Question& question) {
    std::cout << question.text << std::endl;
    for (unsigned i = 0; i < Constants::MAX_ANSWERS_AMOUNT; i++) {
...
Question initQuestion(const char* question, Answer answers[Constants::MAX_ANSWERS_AMOUNT], AnswerOption correct, unsigned points) {
    Question result;
...
void doTest(const Test& test) {
    ...
    for (unsigned i = 0; i < Constants::MAX_QUESTIONS_AMOUNT; i++) {
